...
For simplicity, we assume the indexes of users in \eqref{eq:single_resource_system_model} have been renumbering. 
...
\begin{equation}
 \label{eq:subtract}
    ...
    g_{i}\hat{s}_{i}.
\end{equation}
For BPSK modulation, the log likelihood ratio (LLR) of user $x_i$ can be simplified as
\begin{equation}
    \label{eq:LLR}
    L_{x_i}
    =
...
\end{equation}
where $M$ is the number of coded bits per modulation symbol and $s_l\in\mathcal{S}$ contributes to the signal part in \eqref{eq:single_resource_system_model}. 
...
\end{equation}
Assuming the signals in $\mathcal{S}$ are equally probable, the APPs can be written as
\begin{equation}
...
Figure~\ref{fig:single-resource-ber-1-0-1-0-01} shows the BER performance of three users with large-scale fading coefficients $(\beta_1, \beta_2, \beta_3)=(1,0.1,0.01)$.
