...
For multi-resource PD-NOMA, MMSE-SIC detection is adopted, post-SINR ordering and amplitude-based ordering are considered as two detection-ordering methods, and the BER performance is compared with HDS-CDMA with ML detection~\cite{lin2023gdma}.
...
\begin{equation}
    ...
    \end{bmatrix}.
\end{equation}
The $k$th column $\bm{h}_k$ of $\bm{H}$ represents the resource domain channel vector of user $k$.
For the representative two-resource and three-user case, the received signal is
\begin{equation}
...
The MMSE-SIC receiver provides a low-complexity alternative to joint ML/MAP detection~\cite{zanella2005mmse} by applying a linear MMSE filter and successively cancelling users.
...
After the detection order is decided, users are renumbered as $1,2,\ldots,U$. At stage $i$, the residual vector is $\bm{y}_{(i)}$ and the corresponding channel matrix is
\begin{equation}
...
The soft symbol estimate is then subtracted from the received vector:
\begin{equation}
    \bm{y}_{(i+1)}=\bm{y}_{(i)}-\bm{h}_i\hat{s}_i .
\end{equation}
The process then goes back to re-calculate the coefficient $\bm{w}_{i+1}^H$ by using \eqref{eq:coefficient} with $\bm{H}_{(i+1)}$. The LLR of $x_{i+1}$ and its soft symbol estimate $\hat{s}_{i+1}$ can also be obtained by \eqref{eq:mmse-sic-general-llr} and \eqref{eq:mmse-sic-general-soft-estimate}. This procedure is repeated until all users are detected.
...
The results in figure.~\ref{fig:multi-resource-beta0-1} and \ref{fig:multi-resource-beta1} show that HDS-CDMA with ML detection, which are indicated by the solid lines, achieves better uncoded BER than MMSE-SIC in both large-scale fading cases. For the unequal-$\beta_k$ case in figure.~\ref{fig:multi-resource-beta0-1}, HDS-CDMA performs about $3$ dB better than MMSE-SIC at $\mathrm{BER}=10^{-4}$ for user 1. For the equal-$\beta_k$ case in figure.~\ref{fig:multi-resource-beta1}, HDS-CDMA performs about $7$ dB better than MMSE-SIC at $\mathrm{BER}=10^{-4}$. 
Therefore, the performance loss of MMSE-SIC is much more significant when the users have similar average received powers.

The smaller BER gap of two detector in the unequal-$\beta_k$ case is caused by the large-scale fading disparity. 
...
The detection ordering comparison further explains the behavior of MMSE-SIC. In the unequal-$\beta_k$ case, amplitude-based ordering only causes a slight degradation around $25$ dB for user 1. In the equal-$\beta_k$ case, however, it degrades heavily.
