...
本論文針對上行非正交的多重接取（non-orthogonal multiple access, NOMA）系統進行比較分析。首先，在未編碼單一資源系統中，比較功率域非正交的多重接取（power-domain NOMA, PD-NOMA）與分增益多重存取（gain division multiple access, GDMA）的錯誤率表現。
接著將功率域非正交的多重接取 (PD-NOMA) 延伸至多資源系統模型，並在未編碼傳輸下比較 MMSE-SIC 偵測器與高密度簽名碼分
...
First, uncoded single-resource power-domain NOMA (PD-NOMA) is compared with gain division multiple access (GDMA). PD-NOMA is then extended to a multi-resource model, where MMSE-SIC is evaluated against high density signature CDMA (HDS-CDMA) under uncoded transmission. 
